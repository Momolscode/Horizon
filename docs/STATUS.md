...
_Mis à jour le 2026-09-26 : e-mails « avis retiré » et « revendication refusée » (D-019)._
...
| Tests unitaires | RÉUSSIE | `npm test` | 15 fichiers, 122 tests |
...
| Parcours e2e connectés | RÉUSSIE | `npm run test:e2e:connected` | 17 tests sur Supabase local + `next dev`, dont le Duo, le référencement, le retrait de la gestion, le renoncement, le refus motivé et l'avis retiré, chacun avec son e-mail reçu dans Mailpit |
| Migrations sur base vide | RÉUSSIE | `npm run supabase:reset` | 10 migrations + seed de 40 lieux |
...
| Captures réelles du référencement (connecté, mobile) | RÉUSSIE | `REF_SHOTS=docs/screenshots npm run test:e2e:connected -- e2e/connected-referencing.spec.ts` | 6 captures `ref-*.jpg`, sous `next dev` (`ref-05` et `ref-06` : retrait et renoncement) ; `ref-07` et `ref-08` : e-mails « avis retiré » et « revendication refusée » affichés par Mailpit |
